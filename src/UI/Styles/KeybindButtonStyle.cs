    ...
    RectData ItemRect)
{
    ...
    internal static KeybindButtonStyle? Capture(Transform panelRoot, TextAppearance fallback, Sprite? rowSprite)
    {
        ...
        var primaryRectData = primaryRect is not null ? RectData.From(primaryRect) : DefaultPrimaryRect;
        ...
        var secondaryRectData = secondaryRect is not null ? RectData.From(secondaryRect) : DefaultSecondaryRect;

        var item = anyButton.parent?.parent;
        var itemRect = item?.GetComponent<RectTransform>();
        var itemRectData = itemRect is not null ? RectData.From(itemRect) : DefaultItemRect;
